...
* Special thanks to Jacob Coblnetz (@Jacob_Coblnetz) for sharing his slides on the SQL workshop used in the MIT political science department. 
...
- Medium sized file (1-2 GB)
  ...
  - Tools: 
      - R:`data.table::fread(file path, select = c("column 1", "column 2"))`. This command imports data faster than `read.csv()` does. `vroom::vroom()` is even faster.
...
![Relational database example. Source: MySQL Tutorial](https://sp.mysqltutorial.org/wp-content/uploads/2009/12/MySQL-Sample-Database-Schema.png)
...
This wonderfule zine was created by [Julia Evans](https://jvns.ca/).
...
Your turn: write the same code in SQL 
Hint: `filter(condition1, condition2)` = `WHERE condition1 and condition2`
...
Your turn: write the same code in SQL 
Hint: in SQL the order should be `SELECT group_var1, group_var2, AVG(old_var) AS new_var` -> `FROM` -> `GROUP BY`
